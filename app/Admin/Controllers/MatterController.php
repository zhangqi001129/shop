...
class MatterController extends Controller
{
    ...
    {
        $grid = new Grid(new WxMatter);

        $grid->id('Id');
        $grid->openid('Openid');
        $grid->add_time('Add time');
        $grid->msg_type('Msg type');
        $grid->medis_id('Medis id');
        $grid->format('Format');
        $grid->msg_id('Msg id');
    ...
    {
        $show = new Show(WxMatter::findOrFail($id));

        $show->id('Id');
        $show->openid('Openid');
        $show->add_time('Add time');
        $show->msg_type('Msg type');
        $show->medis_id('Medis id');
        $show->format('Format');
        $show->msg_id('Msg id');
    ...
    {
        $form = new Form(new WxMatter);

        $form->text('openid', 'Openid');
        $form->number('add_time', 'Add time');
        $form->text('msg_type', 'Msg type');
        $form->text('medis_id', 'Medis id');
